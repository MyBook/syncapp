...
def get_maps(text_content_file, audio_path):
    ...
    for i, z in enumerate(parsed.iter()):
        if z.text and z.tag == 'p':
            map_table.append((unicode(i), [z.text_content()], z.getroottree().getpath(z)))
    ...
    textfile = TextFile()
    for identifier, frag_text, xpath in map_table:
    ...
    return task.sync_map.json_string, map_table
